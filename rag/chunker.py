...
def chunk_text(text: str, chunk_size: int = 100, overlap: int = 20) -> list[str]:
    ...
    chunks = []  #a list of sentences
    step = chunk_size - overlap #the step we take each time we put a sentence in the chunks list
    ...
    for i in range(0, len(words), step):
        ...
        if len(chunk)<10: #if chunk is less then ten we stop because a chunk that small is meaningles
            break
        chunks.append(" ".join(chunk))#appending a sentence after each iteration
    return chunks